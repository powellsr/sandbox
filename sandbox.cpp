...
double mp2_energy(const btas::Tensor<double> &ia_jb, const int nocc, const int n, const Eigen::VectorXd &evals);
...
btas::Tensor<double> make_I_ou_ou(const btas::Tensor<double> &v_ou_ou, const btas::Tensor<double> &v_uu_oo,
                                  const btas::Tensor<double> &t_oo_uu, const int nocc, const int n);

btas::Tensor<double> make_I_oo_oo(const btas::Tensor<double> &v_oo_oo, const btas::Tensor<double> &v_uu_oo,
                                  const btas::Tensor<double> &t_oo_uu, const int nocc, const int n);

btas::Tensor<double>
make_I_o_o(const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &t_oo_uu, const int nocc, const int n);

btas::Tensor<double>
make_I_u_u(const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &t_oo_uu, const int nocc, const int n);

double calc_ccd_energy(btas::Tensor<double> const &t_oo_uu, btas::Tensor<double> const &v_oo_uu, const int n, const int nocc);
...
/// CCSD Intermediates
btas::Tensor<double>
make_h_o_o(const btas::Tensor<double> &F_o_o, const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &tau,
           const int ndocc, const int nuocc);

btas::Tensor<double> make_h_u_o(const btas::Tensor<double> &F_u_o, const btas::Tensor<double> &v_uu_oo,
                                const btas::Tensor<double> &singles_amps, const int ndocc, const int nuocc);

btas::Tensor<double>
make_h_u_u(const btas::Tensor<double> &F_u_u, const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &tau,
           const int ndocc, const int nuocc);

btas::Tensor<double> make_g_o_o(const btas::Tensor<double> &h_o_o, const btas::Tensor<double> &F_u_o,
                                const btas::Tensor<double> &singles_amps, const btas::Tensor<double> &v_ou_oo,
                                const int ndocc, const int nuocc);

btas::Tensor<double> make_g_u_u(const btas::Tensor<double> &h_u_u, const btas::Tensor<double> &F_u_o,
                                const btas::Tensor<double> &singles_amps, const btas::Tensor<double> &v_uu_uo,
                                const int ndocc, const int nuocc);

btas::Tensor<double> make_a_oo_oo(const btas::Tensor<double> &v_oo_oo, const btas::Tensor<double> &v_ou_oo,
...
btas::Tensor<double> make_b_uu_uu(const btas::Tensor<double> &v_uu_uu, const btas::Tensor<double> &v_uu_uo,
...
btas::Tensor<double> make_j_ou_uo(const btas::Tensor<double> &v_ou_uo, const btas::Tensor<double> &v_ou_oo,
                                  const btas::Tensor<double> &singles_amps, const btas::Tensor<double> &v_uu_uo,
                                  const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &T,
                                  const btas::Tensor<double> &doubles_amps, const int ndocc, const int nuocc);

btas::Tensor<double> make_k_ou_ou(const btas::Tensor<double> &v_ou_ou, const btas::Tensor<double> &v_ou_oo,
                                  const btas::Tensor<double> &v_uu_ou, const btas::Tensor<double> &singles_amps,
                                  const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &T, const int ndocc,
                                  const int nuocc);

btas::Tensor<double> t1_residual(const btas::Tensor<double> &F_o_u, const btas::Tensor<double> &F_u_o,
                                 const btas::Tensor<double> &singles_amps, const btas::Tensor<double> &h_u_u,
                                 const btas::Tensor<double> &h_o_o, const btas::Tensor<double> &h_u_o,
                                 const btas::Tensor<double> &doubles_amps, const btas::Tensor<double> &v_uo_ou,
                                 const btas::Tensor<double> &v_ou_ou, const btas::Tensor<double> &v_uu_ou,
                                 const btas::Tensor<double> &tau, const btas::Tensor<double> &v_uo_oo, const int nuocc,
                                 const int nocc);

btas::Tensor<double>
t2_residual(const btas::Tensor<double> &v_oo_uu, const btas::Tensor<double> &a_oo_oo, const btas::Tensor<double> &tau,
            const btas::Tensor<double> &b_uu_uu, const btas::Tensor<double> &g_u_u,
            const btas::Tensor<double> &doubles_amps, const btas::Tensor<double> &singles_amps,
            const btas::Tensor<double> &g_o_o, const btas::Tensor<double> &v_ou_uu, const btas::Tensor<double> &v_ou_ou,
            const btas::Tensor<double> &v_oo_uo, const btas::Tensor<double> &v_ou_uo,
            const btas::Tensor<double> &j_ou_uo, const btas::Tensor<double> &k_ou_ou, const int nuocc, const int nocc);
  ...
  try {
    ...
    for (size_t i = 0; i != ndocc; ++i) {
      for (size_t a = 0; a != nuocc; ++a) {
        for (size_t b = 0; b != nuocc; ++b) {
          for (size_t j = 0; j != ndocc; ++j) {
            v_ou_uo(i, a, b, j) = v_gg_gg(i, a + ndocc, b + ndocc, j);
          }
        }
      }
    }
    ...
    for (size_t i = 0; i != ndocc; ++i) {
      for (size_t j = 0; j != ndocc; ++j) {
        for (size_t a = 0; a != nuocc; ++a) {
          for (size_t k = 0; k != ndocc; ++k) {
            v_oo_uo(i, j, a, k) = v_gg_gg(i, j, a + ndocc, k);
          }
        }
      }
    }
...
/*    btas::Tensor<double> I_uo_ou(nuocc, ndocc, ndocc, nuocc);
    ...
    btas::Tensor<double> h_o_o(ndocc, ndocc);
    btas::Tensor<double> h_u_o(nuocc, ndocc);
    btas::Tensor<double> h_u_u(nuocc, nuocc);
    btas::Tensor<double> g_o_o(ndocc, ndocc);
    btas::Tensor<double> g_u_u(nuocc, nuocc);
    btas::Tensor<double> a_oo_oo(ndocc, ndocc, ndocc, ndocc);
...
//    }
    ...
    for (size_t a = 0; a != nuocc; ++a) {
      for (size_t b = 0; b != nuocc; ++b) {
        F_u_u(a, b) = F_mo(a + ndocc, b + ndocc);
      }
    }

    for (size_t i = 0; i != ndocc; ++i) {
      for (size_t a = 0; a != nuocc; ++a) {
        F_o_u(i, a) = F_mo(i, a + ndocc);
      }
    }
...
make_I_u_u(const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &t_oo_uu, const int nocc, const int n) {
  ...
  for (int i = 0; i != nocc; ++i) {
    for (int j = 0; j != nocc; ++j) {
      for (int a = 0; a != nuocc; ++a) {
        for (int b = 0; b != nuocc; ++b) {
          v_temp(a, b, i, j) = (-2.0 * v_uu_oo(a, b, i, j)) + v_uu_oo(b, a, i, j);
        }
      }
    }
  }
...
make_I_o_o(const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &t_oo_uu, const int nocc, const int n) {
  ...
  for (int i = 0; i != nocc; ++i) {
    for (int j = 0; j != nocc; ++j) {
      for (int a = 0; a != nuocc; ++a) {
        for (int b = 0; b != nuocc; ++b) {
          v_temp(a, b, i, j) = (2.0 * v_uu_oo(a, b, i, j)) - v_uu_oo(a, b, j, i);
        }
      }
    }
  }
...
btas::Tensor<double> make_I_uo_ou(const btas::Tensor<double> &v_uo_ou, const btas::Tensor<double> &v_uu_oo,
  ...
  btas::contract(0.5, v_uu_oo, {'e', 'b', 'i', 'm'}, t_oo_uu, {'j', 'm', 'a', 'e'}, 0.0, vt_2ndterm,
                 {'b', 'j', 'i', 'a'});
  ...
  btas::contract(1.0, v_uu_oo, {'b', 'e', 'i', 'm'}, t_temp, {'m', 'j', 'e', 'a'}, 0.0, vt_1stterm,
                 {'b', 'j', 'i', 'a'});
...
calcResidual(const btas::Tensor<double> &v_oo_uu, const btas::Tensor<double> &v_uu_uu, const btas::Tensor<double> &t,
  ...
  for (int i = 0; i != nocc; ++i) {
    for (int j = 0; j != nocc; ++j) {
      for (int a = 0; a != nuocc; ++a) {
        for (int b = 0; b != nuocc; ++b) {
          t_temp(i, j, a, b) = (2.0 * t(i, j, a, b)) - t(j, i, a, b);
        }
      }
    }
  }
  ...
  R = v_oo_uu + t_oo_uu_I_u_u - t_oo_uu_I_o_o + v_uu_uu_t_oo_uu + t_oo_uu_I_oo_oo - t_oo_uu_I_ou_ou - I_ou_ou_t_oo_uu +
      t_oo_uu_I_uo_ou;
  return R;
}
...
double calc_ccd_energy(btas::Tensor<double> const &t_oo_uu, btas::Tensor<double> const &v_oo_uu, const int n, const int nocc) {
  ...
  for (int i = 0; i != nocc; ++i) {
    for (int j = 0; j != nocc; ++j) {
      for (int a = 0; a != nuocc; ++a) {
        for (int b = 0; b != nuocc; ++b) {
          double change = (2.0 * v_oo_uu(i, j, a, b) - v_oo_uu(i, j, b, a)) * t_oo_uu(i, j, a, b);
          ccd_e += change;
        }
      }
    }
...
make_h_o_o(const btas::Tensor<double> &F_o_o, const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &tau,
           const int ndocc, const int nuocc) {
  btas::Tensor<double> v_temp(nuocc, nuocc, ndocc, ndocc);
  for (size_t a = 0; a != nuocc; ++a) {
    for (size_t b = 0; b != nuocc; ++b) {
      for (size_t i = 0; i != ndocc; ++i) {
        for (size_t j = 0; j != ndocc; ++j) {
  ...
  btas::Tensor<double> vTau(ndocc, ndocc);
...
make_h_u_u(const btas::Tensor<double> &F_u_u, const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &tau,
  ...
  btas::Tensor<double> vTau(nuocc, nuocc);
...
btas::Tensor<double> make_h_u_o(const btas::Tensor<double> &F_u_o, const btas::Tensor<double> &v_uu_oo,
  ...
  btas::Tensor<double> vt(nuocc, ndocc);
  btas::contract(1.0, v_temp, {'a', 'b', 'i', 'j'}, singles_amps, {'j', 'b'}, 0.0, vt, {'a', 'i'});
...
btas::Tensor<double> make_g_o_o(const btas::Tensor<double> &h_o_o, const btas::Tensor<double> &F_u_o,
                                const btas::Tensor<double> &singles_amps, const btas::Tensor<double> &v_ou_oo,
                                const int ndocc, const int nuocc) {
  btas::Tensor<double> ft(ndocc, ndocc);
  btas::contract(1.0, F_u_o, {'a', 'i'}, singles_amps, {'u', 'a'}, 0.0, ft, {'u', 'i'});

  btas::Tensor<double> v_temp(ndocc, nuocc, ndocc, ndocc);
  for (size_t u = 0; u != ndocc; ++u) {
    for (size_t a = 0; a != nuocc; ++a) {
      for (size_t i = 0; i != ndocc; ++i) {
        for (size_t j = 0; j != ndocc; ++j) {
          v_temp(u, a, i, j) = (2.0 * v_ou_oo(u, a, i, j)) - v_ou_oo(u, a, j, i);
        }
      }
    }
  }
  btas::Tensor<double> vt(ndocc, ndocc);
...
btas::Tensor<double> make_g_u_u(const btas::Tensor<double> &h_u_u, const btas::Tensor<double> &F_u_o,
                                const btas::Tensor<double> &singles_amps, const btas::Tensor<double> &v_uu_uo,
                                const int ndocc, const int nuocc) {
  btas::Tensor<double> ft(nuocc, nuocc);
  btas::contract(1.0, F_u_o, {'a', 'i'}, singles_amps, {'i', 'B'}, 0.0, ft, {'a', 'B'});

  btas::Tensor<double> v_temp(nuocc, nuocc, nuocc, ndocc);
  for (size_t a = 0; a != nuocc; ++a) {
    for (size_t b = 0; b != nuocc; ++b) {
      for (size_t B = 0; B != nuocc; ++B) {
        for (size_t i = 0; i != ndocc; ++i) {
          v_temp(a, b, B, i) = (2.0 * v_uu_uo(a, b, B, i)) - v_uu_uo(b, a, B, i);
        }
      }
    }
  }
  btas::Tensor<double> vt(nuocc, nuocc);
...
btas::Tensor<double> make_j_ou_uo(const btas::Tensor<double> &v_ou_uo, const btas::Tensor<double> &v_ou_oo,
                                  ...
                                  const btas::Tensor<double> &v_uu_oo, const btas::Tensor<double> &T,
                                  const btas::Tensor<double> &doubles_amps, const int ndocc, const int nuocc) {
  btas::Tensor<double> vt1(ndocc, nuocc, nuocc, ndocc);
  btas::contract(1.0, v_ou_oo, {'u', 'a', 'j', 'i'}, singles_amps, {'j', 'B'}, 0.0, vt1, {'u', 'a', 'B', 'i'});

  btas::Tensor<double> vt2(ndocc, nuocc, nuocc, ndocc);
  btas::contract(1.0, v_uu_uo, {'b', 'a', 'B', 'i'}, singles_amps, {'u', 'b'}, 0.0, vt2, {'u', 'a', 'B', 'i'});

  btas::Tensor<double> vT(ndocc, nuocc, nuocc, ndocc);
  ...
  btas::contract(0.5, v_temp, {'a', 'b', 'i', 'j'}, doubles_amps, {'u', 'j', 'B', 'b'}, 0.0, vtd, {'u', 'a', 'B', 'i'});
...
btas::Tensor<double> t1_residual(const btas::Tensor<double> &F_o_u, const btas::Tensor<double> &F_u_o,
  ...
  btas::contract(2.0, term2_int, {'a', 'B'}, singles_amps, {'u', 'a'}, 0.0, term2, {'u', 'B'});
  ...
  for (size_t a = 0; a != nuocc; ++a) {
    for (size_t i = 0; i != nocc; ++i) {
      for (size_t j = 0; j != nocc; ++j) {
        for (size_t b = 0; b != nuocc; ++b) {
          term6_int(a, i, j, b) = (2.0 * v_uo_ou(a, i, j, b)) - v_ou_ou(i, a, j, b);
        }
      }
    }
  }
  ...
  for (size_t a = 0; a != nuocc; ++a) {
    for (size_t b = 0; b != nuocc; ++b) {
      for (size_t i = 0; i != nocc; ++i) {
        for (size_t c = 0; c != nuocc; ++c) {
          term7_int(a, b, i, c) = (2.0 * v_uu_ou(a, b, i, c)) - v_uu_ou(b, a, i, c);
        }
      }
    }
  }
    ...
    for (size_t i = 0; i != nocc; ++i) {
      for (size_t j = 0; j != nocc; ++j) {
        for (size_t k = 0; k != nocc; ++k) {
          term8_int(a, i, j, k) = (2.0 * v_uo_oo(a, i, j, k)) - v_uo_oo(a, i, k, j);
        }
      }
    }
  }
...
t2_residual(const btas::Tensor<double> &v_oo_uu, const btas::Tensor<double> &a_oo_oo, const btas::Tensor<double> &tau,
  ...
  btas::Tensor<double> term6_int2(nocc, nuocc, nuocc, nuocc);
  for (size_t i = 0; i != nocc; ++i) {
    for (size_t a = 0; a != nuocc; ++a) {
      for (size_t b = 0; b != nuocc; ++b) {
        for (size_t c = 0; c != nuocc; ++c) {
  ...
  btas::Tensor<double> term7(nocc, nocc, nuocc, nuocc);
  btas::contract(1.0, term7_int2, {'u', 'v', 'B', 'i'}, singles_amps, {'i', 'Y'}, 0.0, term7, {'u', 'v', 'B', 'Y'});
  term7 = ccd_permute(term7);

  btas::Tensor<double> term8_int1(nocc, nuocc, nuocc, nocc);
  for (size_t i = 0; i != nocc; ++i) {
    for (size_t a = 0; a != nuocc; ++a) {
      for (size_t b = 0; b != nuocc; ++b) {
        for (size_t j = 0; j != nocc; ++j) {
          term8_int1(i, a, b, j) = (2.0 * j_ou_uo(i, a, b, j)) - k_ou_ou(i, a, j, b);
        }
      }
    }
  }
  ...
  for (size_t i = 0; i != nocc; ++i) {
    for (size_t j = 0; j != nocc; ++j) {
      for (size_t a = 0; a != nuocc; ++a) {
        for (size_t b = 0; b != nuocc; ++b) {
          term8_int2(i, j, a, b) = (2.0 * doubles_amps(i, j, a, b)) - doubles_amps(i, j, b, a);
        }
      }
    }
  }
  btas::Tensor<double> term8(nocc, nocc, nuocc, nuocc);
  btas::contract(0.5, term8_int1, {'u', 'a', 'B', 'i'}, term8_int2, {'i', 'v', 'a', 'Y'}, 0.0, term8,
                 {'u', 'v', 'B', 'Y'});
  term8 = ccd_permute(term8);

  btas::Tensor<double> term9(nocc, nocc, nuocc, nuocc);
  btas::contract(0.5, k_ou_ou, {'u', 'a', 'i', 'B'}, doubles_amps, {'i', 'v', 'Y', 'a'}, 0.0, term9,
                 {'u', 'v', 'B', 'Y'});
  term9 = ccd_permute(term9);

  btas::Tensor<double> term10(nocc, nocc, nuocc, nuocc);
  btas::contract(1.0, k_ou_ou, {'u', 'a', 'i', 'Y'}, doubles_amps, {'i', 'v', 'B', 'a'}, 0.0, term10,
                 {'u', 'v', 'B', 'Y'});
